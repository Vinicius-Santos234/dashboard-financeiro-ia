...
export function LinhaTransacao({
  ...
  // `aria-expanded` sozinho diz que ALGO abre, e não o quê. Sem `aria-controls`
  // apontando para um id, o leitor de tela não tem como levar a pessoa até o
  // painel que acabou de aparecer.
  ...
  // A borda fecha o CONJUNTO, não a linha: com o painel aberto, um traço entre
  // a transação e o painel dela os faria parecer dois itens diferentes.
  const borda = 'border-b border-linha last:border-0'

  return (
    <>
      <tr className={aberta ? undefined : borda}>
        <td className="valor py-4 pr-6 align-baseline text-sm text-fraco">
        ...
        <td
          className="valor py-4 text-right align-baseline text-sm"
          style={
      ...
      {aberta && (
        // O painel é uma LINHA da mesma tabela, atravessando as três colunas.
        // Pô-lo fora da tabela quebraria a associação com a transação que o
        // abriu — e é justamente essa associação que a tabela existe para dar.
